...
export class TypeCheckError extends Error {
  constructor(message: string, readonly token: PseudoToken) {
    super(message);
  }
}
...
const [LOGIC, NUMBER, STRING, NONE] = [
...
export function typeCheck(ast: AST, fMap: FunctionTypeMap, vMap: VariableTypeMap): TypeCheckResult {
  ...
  switch (ast.kind) {
    ...
    case ASTKind.ASSIGNMENT: {
      ...
      if (vType && !compare(vType, eType))
        throw new TypeCheckError(
          `Variable ${ast.variable.name}: Expected ${show(vType)}, but got ${show(eType)}.`,
          ast.token,
        );
    ...
    case ASTKind.CALCBINOP: {
      ...
      if (!compare(t1, NUMBER))
        throw new TypeCheckError(`exp1: Expected NUMBER, got ${show(t1)}`, ast.exp1.token);
      if (!compare(t2, NUMBER))
        throw new TypeCheckError(`exp2: Expected NUMBER, got ${show(t2)}`, ast.exp2.token);
    ...
    case ASTKind.COMPBINOP: {
      ...
      if (!compare(t1, t2))
        throw new TypeCheckError(`${show(t1)} and ${show(t2)} differ!`, ast.exp1.token);
    ...
    case ASTKind.COMPREHENSION: {
      ...
      return {
        type: isHomogenous ? new ArrayType(types[0]) : new HeterogenousArrayType(types),
        fMap,
        vMap,
      };
    }
    ...
    case ASTKind.ARRINDEX: {
      ...
      if (!(vType instanceof ArrayType))
        throw new TypeCheckError(
          `Variable ${ast.variable.name} was not ARRAY, but ${show(vType)}.`,
          ast.token,
        );
    ...
    case ASTKind.ARRASSIGN: {
      ...
      return {
        type: new ArrayType(ast.type === "egész" ? NUMBER : ast.type === "szöveg" ? STRING : LOGIC),
        fMap,
        vMap,
      };
    }
    ...
    case ASTKind.ARRELEMASSIGN: {
      ...
      if (!aType)
        throw new TypeCheckError(`Variable ${ast.arrayIndex.variable.name} not found.`, ast.token);

      if (!(aType instanceof ArrayType)) {
        throw new TypeCheckError(
          `Variable ${ast.arrayIndex.variable.name} expected to be ARRAY, but was ${show(aType)}`,
          ast.token,
        );
      }
    ...
    case ASTKind.FUNCDECL: {
      const types = ast.parameters.map((param) => ({
        name: param.name,
        type: typeCheck(param, fMap, vMap).type,
      }));
      ...
      const nvMap = new Map([
        ...
        ...types.map<[string, Type]>(({ name, type }) => [
          name,
          type instanceof ReferenceType ? type.t : type,
        ]),
      ]);
