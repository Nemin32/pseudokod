...
export class LinearGenerator extends PseudoCodeVisitor {
  ...
  _assemble(ctx, ops) {
    ops.forEach(([opcode, payload, maybe_arg]) => {
      switch (opcode) {
        case "VISIT":
          let arg = Number.isNaN(Number(maybe_arg))
            ? undefined
            : Number(maybe_arg);
  ...
  visitParameterList(ctx) {
    const params = ctx
      .parameterWithType()
      ?.map((p) => this.visitParameterWithType(p));
...
export class LinearExecutor {
  ...
  #skip(opcodes, payload, direction, full = false) {
    ...
    const checkPayload = () =>
      payload == null ? false : this.currentPayload() != payload;
      ...
      case "index":
        {
          ...
          let val = indices.reduce((prev, index) => {
  ...
  stepLine() {
    ...
    while (
      ...
      this.instructions[this.ip].lineNum == current_line
    ) {
      if (this.step()) return;
    }
  }
