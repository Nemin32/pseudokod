...
export const parseReference: P<Reference> = Parser.matchT(TT.REFERENCE).bind(token => parseArrayIndex.or(parseVariable).map(expr => mkToken(token, {tag: "reference", inner: expr})))