...
export function astToDiv(ast: AST): nestedString {
  if (Array.isArray(ast)) {
    ...
    return ([ASTKind[ast.kind]] as nestedString).concat(
      (function (): nestedString {
        switch (ast.kind) {
          ...
          case ASTKind.ARRELEMASSIGN:
            return [
              astToDiv(ast.arrayIndex.variable),
              "[",
              astToDiv(ast.arrayIndex.index),
              "] <- ",
              astToDiv(ast.value),
            ];
...
export function astToString(ast: AST): string {
  if (Array.isArray(ast)) {
    ...
    return `(${ASTKind[ast.kind]} ${(function (ast: Exclude<AST, Block | Parameter[]>) {
      switch (ast.kind) {
        case ASTKind.COMPREHENSION:
        ...
        case ASTKind.ARRELEMASSIGN:
          return `${astToString(ast.arrayIndex.variable)}[${astToString(
            ast.arrayIndex.index,
        ...
        case ASTKind.IF: {
          const head = `(${astToString(ast.headBranch.pred)} => ${astToString(
            ast.headBranch.body,
          )})`;
          const elifs = ast.elIfs
            .map((elIf) => `(${astToString(elIf.pred)} => ${astToString(elIf.body)})`)
            .join(" ");
